...
test('guided tour walks the extended step list with instructions', async ({ page }) => {
  ...
  // 18 steps: welcome, patient, palette, charts, editor, triage, vitals,
  // response, crewscene, handover, summary, board, operators, vault, backup,
  // langpack, conformance, done.
  await expect(card.locator('.tour-step')).toHaveText('1 / 18')
