...
def stack_shot(kicker, title, pairs, accent="ekg"):
    figs = "".join(
        f'<figure class="band"><img src="img/{img}" alt="{esc(cap)}"><figcaption>{esc(cap)}</figcaption></figure>'
        for img, cap in pairs)
    slides.append(f'<section class="slide content">{head(kicker,title,accent)}<div class="stack-shot">{figs}</div></section>')
...
shot_slide("Security · Screenshot","Locked — PHI sealed behind a passphrase",
    "app-vault.png",
    "With the vault on, an idle device auto-locks to this screen: records, the op-log and wound photos are AES-256-GCM encrypted at rest and unreadable until the passphrase is re-entered. In disaster/MCI mode this is mandatory, not optional.",
    accent="amber")
...
stack_shot("Flavor · Humanitarian — screenshots","Built for the multi-team deployment", [
    ("hum-deploybar.png","Deployment context — tag the operation, response type & organization (MCI mode for shared devices)."),
    ("hum-export.png","Donor export — date-range CSV + provenance, plus a retention window (here: 'Keep all')."),
], accent="ekg")
...
.stack-shot{display:flex;flex-direction:column;gap:26px;margin-top:30px;flex:1;justify-content:center}
.stack-shot .band{display:flex;flex-direction:column;gap:9px}
.stack-shot .band img{width:100%;border:1px solid var(--line);border-radius:9px;box-shadow:0 10px 30px rgba(0,0,0,.45)}
.stack-shot figcaption{font-size:15px;color:var(--dim)}
/* footer */
